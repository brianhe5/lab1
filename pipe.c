...
// 		}
// 		else if (childPid == 0) {
// 			// first prog takes stdin and redirects output
// 			// last prog takes redirected input and stdout
// 			// any child in between takes reidrected input
// 			//    and returns redirected output
// 			if (dup2(pipefd[1], 1) == -1) {
// 				perror("pipe redirect err");
// 				exit(errno);
// 			}
// 			if (execlp(argv[i], argv[i], NULL) == -1) {
// 				perror("exec failed; bogus function");
// 				exit(errno);
// 			}
// 		}
// 		else {
...
// 				exit(WEXITSTATUS(status));
// 			} else if (WIFSIGNALED(status)) {
// 				// perror("uncaught signal");
// 				fprintf(stderr, "child signalled with: %d\n", WTERMSIG(status));
// 				exit(WTERMSIG(status));
// 			} else if (WIFSTOPPED(status)) {
// 				// perror("stop signal: %d", WSTOPSIG(status));
// 				fprintf(stderr, "child stoped: %d\n", WSTOPSIG(status));
// 				exit(WSTOPSIG(status));
// 			}
